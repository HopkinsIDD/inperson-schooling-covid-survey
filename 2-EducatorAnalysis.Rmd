...
```{r edtype_recode}
  ...
  df_ws <- df_ws%>%
    mutate(OccEdRecode = OccupationRed,
           OccEdRecode = ifelse(OccEdRecode!="Education", as.character(OccEdRecode),
  ...
  ##Just to simplify the analysis, lets make anything with less than 
  ##50,000 folks other
  df_ws <- df_ws%>%group_by(OccEdRecode)%>%
    mutate(OccEdRecode = ifelse(n()<50000, "Other", OccEdRecode))%>%
    ungroup()
